...
The scoped account deployment also serves the Home, Music, Videos, Support, FAQ, Privacy, and Projects HTML routes so Account navigation is consistent. Existing main-site APIs and assets remain on the main Worker. Profile icons use a fixed six-fish catalogue; Firebase Auth photoURL stores only the canonical icon URL. Both clients validate against the catalogue and fall back to goldfish, without loading arbitrary external profile images.
