...
`npm run build:accounts` builds a standalone static account portal. `npm run deploy:accounts` publishes only the `/account*` routes on Broadway Pixels, preserving the rest of the existing live website. Fishadise.com links to this portal. The general website build also includes the account entry point for its next full-site deployment.
